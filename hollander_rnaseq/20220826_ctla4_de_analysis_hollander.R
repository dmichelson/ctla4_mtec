...
setwd("C:/Users/dmich/Dropbox (MIT)/CBDM Lab MIT/Scratch/20220826_ctla4/hollander_rnaseq")
date = "20220826"

count_table1 <- read.delim( "data/SRR1045015_ReadsPerGene.out.tab", sep="\t", header=T, row.names=1, skip=3 )
count_table2 <- read.delim( "data/SRR1045016_ReadsPerGene.out.tab", sep="\t", header=T, row.names=1, skip=3 )
count_table3 <- read.delim( "data/SRR1045017_ReadsPerGene.out.tab", sep="\t", header=T, row.names=1, skip=3 )
count_table4 <- read.delim( "data/SRR1045018_ReadsPerGene.out.tab", sep="\t", header=T, row.names=1, skip=3 )

count_table <- cbind(count_table3[,1],count_table4[,1],count_table1[,1],count_table2[,1])
rownames(count_table) <- rownames(count_table1)
  
colnames(count_table) <- c("AireWT1","AireWT2","AireKO1","AireKO2")
